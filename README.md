...
- **간편한 실행 환경**: macOS Finder에서 더블클릭만으로 데이터 수집이 가능한 `.command` 실행 파일
...
날짜 출력 시 요일 정보가 포함되도록 텍스트(.txt), HTML, 콘솔 출력 형식을 업데이트했습니다.

### 2026-09-04 (금) · 데이터 소스 교체 (FinanceDataReader)

Yahoo Finance의 데이터 업데이트 지연 문제를 해결하고, 보다 안정적인 데이터 수집을 위해 데이터 소스를 FinanceDataReader로 교체했습니다.

- `kospi_fetch.py`: `yfinance`에서 `FinanceDataReader`로 라이브러리 및 데이터 로직 변경

### 2026-08-30 (일) · 데이터 무결성 강화 (NaN 처리)

Yahoo Finance에서 데이터 업데이트 지연(Data Latency)으로 인해, 최근 거래일에 대해 가격 정보는 누락되고 거래량만 기록되는 현상(NaN)이 발생할 수 있습니다. 이로 인해 통계값이 왜곡되는 것을 방지하기 위해, 데이터 수집 시 유효하지 않은 행을 제외하도록 로직을 개선했습니다.

- `kospi_fetch.py`: `dropna()`를 추가하여 불완전한 데이터 행을 수집 대상에서 제외

### 2026-08-16 (일) · 전일대비 표시 개편
